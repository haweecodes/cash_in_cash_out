...
const naturalCashout = (transaction) => {
  ...
  if (userHistory) {
    ...
    if (transaction.operation.amount > cashOutFee.week_limit.amount) {
      ...
      commissionFee = CommissionCalc(
        transaction.operation.amount - cashOutFee.week_limit.amount,
        cashOutFee.percents
      );
      setUserHistory(
        transaction.user_id,
...
const legalCashout = (transaction) => {
  ...
  if (commissionFee < 0.5) return 0.5;
  return commissionFee;
};
...
const isSameWeek = (transaction, transactionDate) => {
  ...
  userHistoryMapper.set(transaction.user_id, {
    ...userData,
    prevDate: transactionDate,
  });
  return start === end;
